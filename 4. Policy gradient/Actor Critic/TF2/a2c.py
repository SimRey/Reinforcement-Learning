...
class ActorCritic(keras.Model):
    def __init__(self, n_actions, fc1_dims, fc2_dims):
        ...
        self.v = Dense(n_actions, activation=None)
...
class Agent():
    ...
    def learn(self, state, state_, reward, done, n):
        ...
        with tf.GradientTape() as tape:
            probs, Qw = self.actor_critic(state)
            _, Qw_ = self.actor_critic(state_)
            Qw = tf.squeeze(Qw)
            Qw_ = tf.squeeze(Qw_)
            ...
            delta = reward + self.gamma*np.max(Qw_)*(1-done) - Qw[self.action.numpy()[0]]
            advantage = reward + self.gamma*np.max(Qw_)*(1-done) - sum([p*v for p,v in zip(probs, Qw)])


            actor_loss = -(log_probs*advantage)*self.gamma**n
